...
try:
    ...
    raise ImportError(
        "Could not import parser functions from resume_parser.py. "
        "Ensure resume_parser.py defines parse_resume_file and validate_resume_file, "
...
def process_uploaded_resumes(uploaded_files: List[Any], api_key: Optional[str] = None, model_name: Optional[str] = None) -> List[str]:
    messages: List[str] = []
    for uploaded_file in uploaded_files:
        ...
        try:
            metadata = parse_resume_file(str(saved_path), api_key=api_key, model_name=model_name)
...
def show_dashboard_and_explorer(resumes: List[Dict[str, Any]]) -> None:
    st.header("📊 Talent Dashboard & Candidate Explorer")
    
    if not resumes:
        st.info("No resumes uploaded yet. Ingest candidate resumes in the sidebar to get started.")
        return
    ...
    with st.expander("📈 Talent Pool Analytics & Trends", expanded=True):
    ...
    st.markdown("---")
    st.subheader("🔍 Explore Candidate Profiles")
    ...
    with col_f1:
        ...
        selected_name = st.text_input("Filter by Candidate Name")
    with col_f2:
    ...
    for resume in filtered:
        with st.expander(f"👤 {resume.get('candidate_name')} - {resume.get('current_role') or 'Not specified'}"):
            col_info1, col_info2 = st.columns(2)
            with col_info1:
                st.markdown(f"**Company:** {resume.get('current_company') or 'Not specified'}")
                st.markdown(f"**Experience:** {resume.get('experience_years')} years")
                st.markdown(f"**Education:** {resume.get('education') or 'Not specified'}")
            with col_info2:
                st.markdown(f"**Email:** {resume.get('email') or 'N/A'}")
                st.markdown(f"**Phone:** {resume.get('phone') or 'N/A'}")
                st.markdown(f"**Skills:** {', '.join(resume.get('skills', [])) or 'None'}")
                st.markdown(f"**Certifications:** {', '.join(resume.get('certifications', [])) or 'None'}")
            ...
            if projs:
                ...
                st.write("No projects specified.")
                
            if st.button(f"🗑️ Delete Candidate {resume.get('candidate_name')}", key=f"del_{resume.get('id')}"):
                from database import delete_resume, delete_resume_from_vector_store
                db_success, db_msg = delete_resume(resume.get('id'))
                vec_success, vec_msg = delete_resume_from_vector_store(resume.get('id'))
                if db_success:
                    st.success(f"Successfully deleted candidate.")
                    st.rerun()
                else:
                    st.error(f"Error: {db_msg}")


def show_ai_screening_and_ranking(resumes: List[Dict[str, Any]], api_key: str, model_name: str) -> None:
    st.header("🎯 AI Screening & Candidate Ranking")
    
    if not resumes:
        st.info("No candidates available. Ingest resumes to screen.")
        return
        
    tab1, tab2, tab3 = st.tabs(["Candidate Ranking & Export", "AI Talent Intelligence Report", "Contextual Semantic Search"])
    
    with tab1:
        job_description = st.text_area("Paste the Job Description to rank candidates", height=200, key="rank_jd")
        if st.button("Rank Candidates", key="btn_rank") and job_description:
            with st.spinner("Scoring candidates..."):
                ...
                df_ranked = pd.DataFrame(ranking_results)[["candidate_name", "score", "label", "skill_match", "experience_match", "education_match", "semantic_similarity"]]
    ...
    with tab2:
        ...
        selected_candidate = st.selectbox("Select Candidate", options=candidate_options, key="intel_cand")
        intel_jd = st.text_area("Job Description", height=200, key="intel_jd")
        
        if st.button("Generate Talent Intelligence Report", key="btn_intel"):
            ...
            try:
                if api_key and intel_jd:
                    with st.spinner("Generating insights from OpenRouter..."):
                        summary = generate_candidate_intelligence(
                            chosen.get("resume_text", ""), intel_jd, api_key, model_name
                        )
                else:
                    st.info("No API key provided or empty Job Description. Using template-based analysis fallback.")
                    summary = fallback_candidate_summary(chosen.get("resume_text", ""), intel_jd)
                st.markdown(summary)
            except Exception as e:
                st.error(f"Error: {e}")

    with tab3:
        query = st.text_input("Search Query for Semantic Candidate Search", key="search_query")
        if st.button("Run Semantic Search", key="btn_search") and query:
            results = rag_engine.retrieve_documents(query, top_k=5)
            st.write(f"Found {len(results)} contextual resume segments")
            for item in results:
                st.markdown(f"**Resume ID:** {item['resume_id']} (Score: {item['score']:.3f})")
...
def show_rag_chat(api_key: str, model_name: str) -> None:
    ...
    prompt = st.text_area("Ask the talent intelligence assistant a question", key="chat_prompt")
    if st.button("Submit Chat Query", key="btn_chat") and prompt:
        if not api_key:
...
def show_database_admin() -> None:
    st.header("🗄️ Database Administration")
    tab1, tab2 = st.tabs(["Browse Tables", "System Cleanup"])
    ...
    with tab2:
        st.warning("⚠️ **DANGER ZONE** - This will permanently delete all data from the database and vector index.")
        if st.button("🔴 CLEAR ALL SYSTEM DATA permanently", key="btn_clear_all"):
...
def main() -> None:
    ...
    api_key = st.sidebar.text_input("API Key", type="password", help="Your OpenRouter API key")
    model_name = st.sidebar.selectbox("Language Model", MODEL_OPTIONS, index=0)
    
    st.sidebar.markdown("### 📄 Resume Ingestion")
    uploaded_files = st.sidebar.file_uploader("Upload candidate resumes (PDF)", type=["pdf"], accept_multiple_files=True)
    if st.sidebar.button("Process Resumes"):
        if uploaded_files:
            messages = process_uploaded_resumes(uploaded_files, api_key=api_key, model_name=model_name)
    ...
    st.markdown("A unified platform for resume ingestion, AI parsing, semantic search, ranking, and reporting.")

    if page == "Dashboard & Explorer":
        show_dashboard_and_explorer(resumes)
    elif page == "AI Screening & Ranking":
        show_ai_screening_and_ranking(resumes, api_key, model_name)
